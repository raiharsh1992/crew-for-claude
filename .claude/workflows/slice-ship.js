export const meta = {
  name: 'slice-ship',
  description: 'Run ONE slice through the full quality pipeline as a DETERMINISTIC script: implement -> critic -> fixer -> auditor -> verify (script-run) -> open PR. The verification gate is run BY THE SCRIPT, not by a lead reporting "done" — so it cannot be faked. Directly attacks the unverified-parallelism / faked-smoke failure mode.',
  whenToUse: 'When you want a single well-scoped slice taken end-to-end with the verify gate structurally enforced. NOT a replacement for interactive dev-lead work on exploratory slices — use this for clear-scope slices where you want the pipeline guaranteed. Works for any repo type (service / UI / lib).',
  phases: [
    { title: 'Resolve', detail: 'discovery agent classifies the target repo (RESOLVER.md)' },
    { title: 'Implement', detail: 'dev-lead implements the slice in an isolated worktree' },
    { title: 'Review', detail: 'critic finds issues, fixer closes them' },
    { title: 'Gate', detail: 'auditor verdict + verify (script-run, un-fakeable) -> PR' },
  ],
}

const DRY_RUN = !(args && args.dryRun === false)

// args.slice = { repo, branch?, brief, design?, scope }
const slice = (args && args.slice) || null
if (!slice || !slice.repo || !slice.brief || !slice.scope) {
  return { error: 'slice-ship requires args.slice = { repo, branch?, brief, design?, scope }. Aborting — nothing done.' }
}
const short = slice.repo.replace(/-service$/, '').replace(/-/g, '-')
const branch = slice.branch || `feat/${short}-slice`

// ── Resolve the target repo (RESOLVER.md) — its path/class/language drive the
//    verify command, which is read from the skills-config verify map (no
//    hardcoded toolchain here). ──
const RESOLVE_SCHEMA = {
  ...
  properties: {
    ...
    verifyCommand: { type: 'string', description: 'the resolved verify command for this repo from skills-config.json (byRepo.overrides.verify -> defaults[language].verify -> built-in fallback)' },
  },
}
// IMPL_SCHEMA — the dev-lead's structured report of what the slice built. Required
// because EVERY resumingAgent call must pass a schema (withResume(undefined) throws).
const IMPL_SCHEMA = {
  type: 'object', additionalProperties: false,
  required: ['summary'],
  properties: {
    summary: { type: 'string', description: 'what was built (files, endpoints, migrations)' },
...
async function resumingAgent(basePrompt, opts) {
...
const DEDUP_KEY_PREFIX = 'AUTOFILE:'
...
const DEDUP_LABELS = { quarantine: 'triage:needs-review', repo: null }
...
async function fileFindingsOrBlock(findings, opts) {
  opts = opts || {}
...
async function finishDoerOrBlock(opts) {
  opts = opts || {}
...
phase('Resolve')
log(`slice-ship: ${slice.repo} on ${branch} — DRY_RUN=${DRY_RUN}`)
const target = await resumingAgent(
  `You are the Resolve-phase discovery agent for ONE repo: "${slice.repo}". Per .claude/workflows/RESOLVER.md, resolve its path, generic class (SERVICE/LIBRARY/UI/INFRA) and language (args -> repos.config.js -> .gitmodules/markers). Then resolve its VERIFY COMMAND from .claude/skills-config.json: verify.byRepo["${slice.repo}"].overrides.verify, else verify.defaults[<language>].verify, else the built-in fallback for that language. Return {name, path, class, language, verifyCommand}. Read-only.`,
  { label: 'resolve', phase: 'Resolve', schema: RESOLVE_SCHEMA, agentType: 'Explore' }
)
if (!target || !target.path) {
  return { error: `Resolve could not locate repo "${slice.repo}". Check repos.config.js / skills-config.json.`, target }
}
const verifyInstruction = target.verifyCommand
  ? `Execute this repo's CI-equivalence verify command (resolved from skills-config.json) from the repo root:\n    ${target.verifyCommand}\nReport green=true ONLY if every stage passes.`
  : `Resolve this repo's verify command from .claude/skills-config.json (verify.byRepo["${slice.repo}"].overrides.verify -> verify.defaults[language].verify -> the language fallback) and run it from the repo root. Report green=true ONLY if every stage passes.`

// ---- Phase 2: Implement (isolated worktree; dev-lead owns the critical path) ----
phase('Implement')
const impl = await resumingAgent(
  `You are the dev-lead for a slice in repo "${slice.repo}" (path "${target.path}", class ${target.class}${target.language ? `, language ${target.language}` : ''}). Work in an ISOLATED worktree on branch "${branch}". Read docs/AGENT-WORKFLOW.md, the repo's CLAUDE.md / CONTEXT.md, and any design pack first.

BRIEF: ${slice.brief}
${slice.design ? `DESIGN: ${slice.design}` : ''}
SCOPE (in/out): ${slice.scope}

Implement the critical path yourself. Honor every rule in your project's CLAUDE.md. Update CONTEXT.md in the SAME change if tables/endpoints/events/contracts change (binding freshness rule). Stage by explicit name (no git add -A). Commit on the branch. Report what you built (files, endpoints, migrations).`,
  { label: `impl:${short}`, phase: 'Implement', isolation: 'worktree', schema: IMPL_SCHEMA, agentType: 'dev-lead' }
)

// ---- Phase 3: Review (critic -> fixer) ----
phase('Review')
const criticReport = await resumingAgent(
  `Critic pass on the slice just implemented in "${slice.repo}" (branch "${branch}"). Review the full diff (default-branch..${branch}) for: your project's CLAUDE.md rule compliance, spec coverage vs the brief, correctness, security, secrets, and test coverage (golden path + edge per public fn). Produce a structured issue list (C-IDs, severity, file:line, suggested fix), summarized in the schema. BRIEF for reference: ${slice.brief}`,
  { label: 'critic', phase: 'Review', schema: VERDICT_SCHEMA, agentType: 'critic' }
)
const fixReport = await resumingAgent(
  `Fixer pass on "${slice.repo}" branch "${branch}", in the same isolated worktree. Resolve every issue from the critic list below, in severity order, NO scope creep. Re-run any verifiable check. Commit fixes on the branch. Then report what you fixed, summarized in the schema.\n\nCRITIC FINDINGS:\n${JSON.stringify(criticReport)}`,
  { label: 'fixer', phase: 'Review', isolation: 'worktree', schema: VERDICT_SCHEMA, agentType: 'fixer' }
)

// ---- Phase 4: Gate (auditor + finishDoerOrBlock; PR ONLY on APPROVED) ----
phase('Gate')
const audit = await resumingAgent(
  `Auditor — final independent verdict on "${slice.repo}" branch "${branch}". Do NOT trust the fixer report; verify against the actual files. Re-read the brief, confirm every critic issue is genuinely fixed, run a fresh top-down review (your project's CLAUDE.md rules + correctness + spec). List any project CLAUDE.md rule violations in sacredRuleViolations. Issue APPROVED / CONDITIONAL / REJECTED.\n\nBRIEF: ${slice.brief}\nFIXER REPORT: ${JSON.stringify(fixReport)}`,
  { label: 'auditor', phase: 'Gate', schema: VERDICT_SCHEMA, agentType: 'auditor' }
)

// finishDoerOrBlock: runs its own verifier (tester), script-asserts the proof,
// opens PR ONLY on verdict===APPROVED (CONDITIONAL does NOT open the gate).
const finish = await finishDoerOrBlock({
  verdict: audit,
  service: short,
  branch,
  dryRun: DRY_RUN,
  verifyInstruction,
  contextNeeded: true,
  prTitle: `[${short}] ${branch}`,
  prBodyHint: 'scope, auditor APPROVED, verify GREEN, gate tier',
  baseBranch: 'the default branch',
  label: short,
})

return { dryRun: DRY_RUN, repo: slice.repo, branch, auditor: audit, finish }
